...
class BodyMetrics:
    ...
    def __weight_converter(self, weight, unit):
        while True:
            try:
                #weight = input("Weight: ")
                if not weight:
            ...
            if unit.upper() in ["L", "LBS"]:
                self.__res_kgs = weight * 0.45

            elif unit.upper() in ["K", "KG"]:
                self.__res_lbs = weight / 0.45
            else:
                print("Please enter either 'k' or 'l' into a unit input")
                continue
                
            return self.__res_kgs, self.__res_lbs
        ...
        try:
            while True:
                ...
                if choice == "1":
                    ...
                    weight = input("Weight: ")
                    unit = input("Lbs or Kg: ")
                    self.__weight_converter(weight, unit)
                    if unit.upper() in ["L", "LBS"]:
                        print(f"{self.__res_kgs} kgs")
                    elif unit.upper() in ["K", "KG"]:
                        print(f"{self.__res_lbs} pounds")
